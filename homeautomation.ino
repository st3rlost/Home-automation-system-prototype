...
const char *password = "Sacio@2k4";

// Create AsyncWebServer object on port 80
AsyncWebServer server(80); // port 80 is the default port for web traffic 

// Create an Event Source on /events
AsyncEventSource events("/events");

struct device {  // define a structure for all the devices info 
  const int id;
  const int pin;
  const int btnPin;
  int prevBtnState;
...
const char index_html[] PROGMEM = R"rawliteral(
  ...
  <head>
    ...
    <title>ESP32 based Home Automation</title>
    <style>
  ...
  <body>
      <h1>ESP32 SMART Home controls !</h1>
...
void setup() {
  ...
  Serial.begin(115200); //initialize serial monitor for debugging and all

  pinMode(d1.pin, OUTPUT); // syntax is pinMode(pin,mode (output,input,input_pullup (defines internal pull up resistor)))
  ...
  WiFi.mode(WIFI_STA); // used to help esp connect to a wifi networkWIFI_STA → Station mode (ESP acts as a Wi-Fi client).WIFI_AP → Access Point mode (ESP acts as a Wi-Fi hotspot).WIFI_AP_STA → Dual mode (ESP acts as both client and hotspot).
  ...
  // Handle Web Server Root // HTTP GET /update request 
